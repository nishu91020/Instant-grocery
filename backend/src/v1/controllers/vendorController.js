...
const { addNewVendor, removeVendorById, getAllVendors, findVendorById, findVendorByIdAndUpdate } = require('../services/vendorServices');
...
exports.addVendor = async (req, res, next) => {
    try {
        const REQUIRED = [ 'firstName', 'lastName', 'email', 'password', 'businessPhoneNo', 'businessName', 'gstin', 'address' ];

        const vendorData = validateRequestBody(REQUIRED, req.body);

        const address = new Address(vendorData.address);
        await address.save();
        vendorData.address = address;

        // vendor.address.contactNumber = vendorData.businessPhoneNo;
        ...
        const token = await generateToken(vendor);
